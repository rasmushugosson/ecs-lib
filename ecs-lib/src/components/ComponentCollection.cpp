...
ae::ComponentCollection::ComponentCollection()
	: m_Pools(), m_pSystems(), m_SystemsLookup(), m_Entities()
{
}
...
void ae::ComponentCollection::RemoveSystem(ComponentSystem* pSystem)
{
	...
	{
		AE_LOG_CONSOLE(AE_WARNING, "Tried to remove system from component collection but failed to find the system. This should never happen, please report as a bug.");
		return;
	}
